#!/usr/bin/env python3
"""
Dataset Collector - Architecture DISTRIBUEE
Collecte les métriques de l'architecture distribuée uniquement.
...
# ─────────────────────────────────────────────
# CONFIGURATION DISTRIBUEE
# ─────────────────────────────────────────────
API_URL = "http://localhost:8000/api/query/distributed"
CSV_PATH = "dataset_distribuee.csv"
SESSION_BASE = "session_dist_"
...
def call_api(question: str, session_id: str) -> dict:
    ...
    try:
        ...
        return {"error": "timeout", "temps_ms": TIMEOUT_MS}
    except Exception as e:
...
class DistributedCollector:
    ...
    def _save_counter(self):
        with open("collector_dist_meta.json", "w") as f:
    ...
    def collect(self, question: str, notes: str = "") -> dict:
        ...
        print(f"[DIST] {status} {raw['temps_ms']}ms | {raw.get('tokens_total',0)} tokens | qualité={quality} | score={score}")
...
def main():
    print("╔══════════════════════════════════════════════════════════════════╗")
    print("║   Dataset Collector — ARCHITECTURE DISTRIBUEE                    ║")
    print("║   Évaluateur: NVIDIA Llama-3.1-70B-Instruct                      ║")
    print("║   Timeout API: 180 secondes                                      ║")
    print("║   Fichier: dataset_distribuee.csv                                ║")
    print("╚══════════════════════════════════════════════════════════════════╝\n")
